...
class MovieModel extends Model
{
    ...
    public function edit($movie_id)
    {
        ...
        $data = [];
        $temp_data = [];
        $sql = "SELECT * FROM `movie_cinemas` WHERE movie_id = '" . $movie_id . "' AND is_deleted = 0";
        $result = mysqli_query($this->connection, $sql);
        while ($row = mysqli_fetch_object($result))
        {
            $exists = false;
            foreach ($un_updated_cinemas as $un_updated_cinema)
            {
                if ($un_updated_cinema["cinema"] == $row->cinema_id)
                {
                    $exists = true;
                    break;
                }
            ...
            if (!$exists)
            {
                $sql = "UPDATE `movie_cinemas` SET is_deleted = 1 WHERE id = '" . $row->id . "'";
                // mysqli_query($this->connection, $sql);

                array_push($temp_data, $sql);
            }
        ...
        if ($previous_cinemas > count($cinemas))
        {
            ...
            // $sql = "UPDATE `movie_cinemas` SET is_deleted = 1 WHERE movie_id = '" . $movie_id . "' AND cinema_id NOT IN (" . implode(',', $updated_cinemas) . ")";
            // mysqli_query($this->connection, $sql);
        }

        $sql = "SELECT * FROM `movie_cinemas` WHERE movie_id = '" . $movie_id . "' AND cinema_id NOT IN (" . implode(',', $updated_cinemas) . ")";
        $result = mysqli_query($this->connection, $sql);
    ...
    public function get_detail($movie_id)
    {
        ...
        $sql = "SELECT *, movie_cinemas.id AS movie_cinema_id FROM movie_cinemas INNER JOIN cinemas ON movie_cinemas.cinema_id = cinemas.id WHERE movie_cinemas.movie_id = '" . $movie_id . "' AND movie_cinemas.is_deleted = 0";
        $result = mysqli_query($this->connection, $sql);
        $cinemas = array();
        while ($row = mysqli_fetch_object($result))
        {
